# Christina Raw 25/3/22

# In this script I am going to prepare the data set for the new modelling approach.
# Instead of using the data in percentage change format, I will use it in LRR 
# format.

# 1. Add sample size = 1 for reviews; number of studies for meta-analyses
# 2. Percentage -> LRR
# 3. Data set from wide -> long
# 4. Reorder columns to make data frame more clear
...
# 2. Transform data from percentage change into LRR
    ...
    # a. Subset LRR and percentage change data
...
unique(LRR$Effect_size)
    ...
    # b. Transform the percentage change data into LRR
...
# to switch control and treatment and then turn (e.g.) -30 into +30
...
perc <- relocate(perc, agricultural_system,   # Relocate columns so they match
                 .after = Control)            # in the rbind
...
# 3. Collapse Treatment (and agricultural_system) levels

d$agricultural_system[d$agricultural_system == "traditional"] <- "Traditional"
d$agricultural_system[d$agricultural_system == "Conventional_NonWeeded"] <- "Conventional"
d$agricultural_system[d$agricultural_system == "Conventional_Weeded"] <- "Conventional"
d$agricultural_system[d$agricultural_system == "Fragmented_forest"] <- "Disturbed_forest"
d$agricultural_system[d$agricultural_system == "Logged_forest"] <- "Disturbed_forest"
d$agricultural_system[d$agricultural_system == "Fragmentded_forest"] <- "Disturbed_forest"
d$agricultural_system[d$agricultural_system == "Primary_forest"] <- "Primary_vegetation"
d$agricultural_system[d$agricultural_system == "Contiguous_forest"] <- "Primary_vegetation"
d$agricultural_system[d$agricultural_system == "Intercrop"] <- "mixed"
d$agricultural_system[d$agricultural_system == "Simplified"] <- "Conventional"
...
write_xlsx(d, "Datasets/06.Excel_Dataset_to_model_LRR_WIDE.xlsx")
...
write_xlsx(long_LRR, "Datasets/07.Excel_Dataset_to_model_LRR_LONG.xlsx")
