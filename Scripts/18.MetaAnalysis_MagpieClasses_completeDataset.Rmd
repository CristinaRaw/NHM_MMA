---
title: "18.MetaAnalysis_MagpieClasses_completeDataset"
...
I am going to get estimates for any commodity that has more than 10 observations for any of the production systems.  The rule of thumb for meta-analyses is 10 observations per level (in this case per commodity). In this case , all commodities have more than ten observations for an agricultural system.
...
-   Log response ratio: 48 observations
-   Percentage change: 287 observations
-   Hedge's g: 171 observations
-   Plain species richness and abundance

```{r, include = FALSE, eval=FALSE}

write.csv(Percentage_change, here("Data/01.Processed_Data/05.Analysis/01.Magpie", "04.Percentage_change.csv"))
write.csv(Percentage_change, here("Outputs/07.Analysis/01.Magpie", "04.Percentage_change.csv"))
write.csv(Hedges_d, here("Data/01.Processed_Data/05.Analysis/01.Magpie", "05.Hedges_d.csv"))
write.csv(Hedges_d, here("Outputs/07.Analysis/01.Magpie", "05.Hedges_d.csv"))
write.csv(LRR, here("Data/01.Processed_Data/05.Analysis/01.Magpie", "06.LRR.csv"))
write.csv(LRR, here("Outputs/07.Analysis/01.Magpie", "06.LRR.csv"))
...
Percentage_change <- read.csv( here("Outputs/07.Analysis/01.Magpie",                      # Load data
                                    "04.Percentage_change.csv"), row.names = 1)

LRR <- read.csv(here("Data/01.Processed_Data/05.Analysis/01.Magpie", "06.LRR.csv"), row.names = 1)
...
Percentage_change <- subset(Percentage_change, magpie_class != "Other perennial crops")
...
magpie_class <- unique(Percentage_change$magpie_class)  # Make list that splits data per crop type 
magpie_list<- list()  
...
oil_crops <- `Oil crops`[!is.na(`Oil crops`$Percentage_change),]
mean_oil <- oil_crops %>%
...
other_annual <- `Other annual crops`[!is.na(`Other annual crops`$Percentage_change),]
mean_annual <- other_annual %>%
                  group_by(agricultural_system) %>%
                  summarise(wheighted.mean = 
                              weighted.mean(Percentage_change, weight)) %>%
                  cbind( crop = "Other annual crops" )

cereals <- Cereals[!is.na(Cereals$Percentage_change),]
...
sugar <- `Sugar Crops`[!is.na(`Sugar Crops`$Percentage_change),]
mean_sugar <- sugar %>%
                  drop_na(Percentage_change) %>%
                  group_by(agricultural_system) %>%
                  summarise(wheighted.mean = 
...
mean_effect <- rbind( mean_oil, mean_cereals, mean_annual, mean_sugar)

```

```{r, eval = FALSE, include = FALSE}

write.csv(mean_effect, here("Data/01.Processed_Data/05.Analysis/01.Magpie", "07.CompleteDataMeanEffect.csv"))
write.csv(mean_effect, here("Outputs/07.Analysis/01.Magpie", "07.CompleteDataMeanEffect.csv"))
...
perc_to_lrr <- LRR[9:13, 38]  # Subset % change data to be back transformed

perc_to_lrr <- log((perc_to_lrr/100) +1)  # Back transform into LRR

LRR[9:13, 37] <- perc_to_lrr  # Input values in the LRR data frame
```
...
LRR$weight[c(1:48)] <- LRR$N_Studies[c(1:48)]  # Add number of studies as weight
...
# Save for code 19 purposes

write.csv(mean_cereals, here("Data/01.Processed_Data/05.Analysis/01.Magpie", "07.LRR_CompleteDataMeanEffect.csv"))
