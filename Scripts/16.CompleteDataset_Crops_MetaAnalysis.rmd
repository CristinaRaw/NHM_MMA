---
title: "16.CompleteDataset_Crops_MetaAnalysis"
...
LRR <- read.csv(here("Data/01.Processed_Data/05.Analysis", "06.LRR.csv"), row.names = 1)
...
LRR$weight[c(1:34)] <- LRR$N_Studies[c(1:34)]  # Add number of studies as weight
...
straw <- subset(LRR, LRR$Crop == "Straw_cereals")
...
mean_straw <- straw %>%
                ...
                summarise(wheighted.mean = 
                            weighted.mean(Value, weight)) %>%
                cbind(crop = "Straw cereals")

mean_effect <- rbind( mean_maize, mean_straw)  # Rbind into one data frame
...
mean_effect <- mutate(mean_effect, Percentage = 100 * (exp(wheighted.mean) - 1))

```

```{r, include = FALSE}

# Save, for script 19 purposes

write.csv(mean_effect, here("Data/01.Processed_Data/05.Analysis", "07.LRR_CompleteDataMeanEffect.csv"))
...
kable(mean_effect, row.names = TRUE, caption = "LRR weighted mean")
...
LRR_plot <- ggplot( mean_effect, aes(wheighted.mean, size = 1, fill = agricultural_system)) +
  geom_dotplot(dotsize = 0.5) +
...
perc_plot <- ggplot( mean_effect, aes(Percentage, size = 1, fill = agricultural_system)) +
  geom_dotplot(dotsize = 0.5) +
  facet_wrap (~ crop, nrow = 3, ncol = 1) +
